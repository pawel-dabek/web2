...
function DCGMapInit(element = document) {
  ...
  const locationXXX = { lat: 31.29631, lng: 47.32504 };
  const mapOptions = {
    zoom: 13.5,
    center: locationXXX,
    styles: mapStyle,
...
function init_contact_block(element = document) {
  function addScript() {
...
/* -- Admin JS START -- */
if (window.acf) {
  window.acf.addAction('render_block_preview/type=contact-block', (el) => {
    if (
      !!el[0].querySelector('.contact-block') &&
      !el[0]
        .querySelector('.contact-block')
        .classList.contains('block-js-added')
    ) {
      init_contact_block(el[0]);
      el[0].querySelector('.contact-block').classList.add('block-js-added');
    }
  });
}
/* -- END -- */
